...
export default {
  ...
  methods: {
    ...
    onAuthorSubmit() {
      ...
      axios
        .post(`/api/publish-paper/${pid}`, this.papers.authors)
        .then((res: any) => {
          this.author_editable = false
        ...
        .catch((err: any) => {
          ElMessage.error({
            showClose: true,
            message: `论文信息更新失败，${err.response.data.msg ?? err}`
          })
        })
    },
    onPaperReset() {
  ...
  <div class="blocktext Plaintext">
    ...
    <el-table :data="papers.authors" max-height="500">
      <el-table-column label="作者排名">
        <template #default="scope">
          <number-input
            v-if="author_editable"
            class="inputli"
            v-model="scope.row.ptrank"
            placeholder="请输入作者排名"
            @blur="sortAuthor(scope.$index)"
          />
      ...
      <el-table-column label="是否通讯作者">
        <template #default="scope">
          <el-tag
            ...
            @click="author_editable && handleCorrespond(scope.$index, false)"
            >通讯作者</el-tag
          >
          <el-tag v-else type="info" @click="author_editable && handleCorrespond(scope.$index)"
            >非通讯作者</el-tag
          >
        </template>
      </el-table-column>
      <el-table-column label="操作" fixed="right" width="100">
        <template #default="scope">
          <danger-button
            ...
            label="删 除"
            message="是否删除该作者？"
            @commit="removeAuthor(scope.$index)"
          />
